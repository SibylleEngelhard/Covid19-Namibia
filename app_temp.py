...
with col3b:
	...
	st.write('Seven Day Incidence: '+str(sevenday_incidence))
	st.write('Seven Day Incidence Windhoek: '+str(sevenday_incidence_whk))
...
with col3c:
	...
	expander2.write(new_hospital_cases)
...
with col6b:
	...
	chart_hospital_cases=alt.Chart(hospital_range_df,height=250).mark_line().encode(
		...
		color=alt.Color('Hospital Cases',
			legend=alt.Legend(
				...
				orient="top"),
			scale=alt.Scale(
				range=['lightgreen','darkorange','green', 'red']))
    ...
    #                  alt.Color('Hospital Cases:N', legend=None),
     ##                 alt.value('lightgray')
     #                 )
# main chart code
	ts_chart = alt.Chart(hospital_range_df,height=250).encode(
    	x=alt.X('hospital_date:T',title='Date'),
		y='Cases',
    	color=alt.Color('Hospital Cases',
                    legend=alt.Legend(
	...
	st.altair_chart(ts_chart.mark_line())
...
with col7a:
	...
	if selected_district=='Windhoek':
		st.write('Seven Day Incidence: '+str(sevenday_incidence_whk))
